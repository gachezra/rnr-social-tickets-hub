...
const TicketStatusPage: React.FC = () => {
  ...
  const handleSearch = async (query: string, type: "email" | "ticketId") => {
    ...
    try {
      ...
      toast.error(
        "There was an error searching for your tickets. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <div className="max-w-3xl mx-auto">
            <div className="text-center mb-8">
              ...
              <p className="text-muted-foreground">
                Enter your email address or ticket ID to check your reservation
                status
              </p>
            </div>
                ...
                {tickets.map((ticket) => {
                  ...
                  if (!event) {
                    return (
                      <div
                        key={ticket.id}
                        className="text-center py-4 bg-card border border-border rounded-lg"
                      >
                        <p className="text-muted-foreground">
